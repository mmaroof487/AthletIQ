...
const Dashboard = ({ user }) => {
	...
	const fetchUserProfile = async () => {
		try {
			...
			setFitness((prev) => ({
				...prev,
				historyWeight: data.weightHistory,
	...
	const workoutStats = {
		...
		workoutsByType:
			fitness.fitnessData?.workouts.reduce((acc, workout) => {
				...
				<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.1 }}>
					<Card className="flex items-center">
						...
						<div>
							...
							<p className="text-2xl font-bold">{fitness.data?.weight || 0} kg</p>
						</div>
					</Card>
				</motion.div>
				...
				<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.2 }}>
					<Card className="flex items-center">
						...
						<div>
							...
							<p className="text-2xl font-bold">{fitness.data?.weightchange || 0} kg</p>
						</div>
					</Card>
				</motion.div>
				...
				<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.3 }}>
					<Card className="flex items-center">
						...
						<div>
							...
							<p className="text-2xl font-bold">{fitness.data?.calorieintake || 0}</p>
						</div>
					</Card>
				</motion.div>
				...
				<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.4 }}>
					<Card className="flex items-center">
						...
						<div>
							...
							<p className="text-2xl font-bold">{fitness.cal || 0}</p>
						</div>
					</Card>
				</motion.div>
			</div>

			<Graphs historyWeight={fitness.historyWeight} historyCalorie={fitness.historyCalorie} />

			<div className="relative my-8">
				<div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2  text-xl bg-primary-500 px-4 py-1 rounded-md z-50 mt-40">Upcoming features</div>
					...
					<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.7 }} className="md:col-span-2">
						<Card title="Recent Workouts">
							<div className="space-y-4 h-64">
								{fitness.fitnessData?.workouts.length ? (
									fitness.fitnessData.workouts
										.slice()
										.reverse()
										.map((workout, index) => (
